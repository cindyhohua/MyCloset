    ...
    @IBAction func signupButtonTapped(_ sender: AnyObject) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authResult, error) in
            if let error = error {
                ...
                let author = Author(email: email, id: authResult?.user.uid ?? "", name: nickName, image: "", height: "", weight: "", privateOrNot: false, littleWords: "", following: [], followers: [], pending: [], post: [], saved: [])
